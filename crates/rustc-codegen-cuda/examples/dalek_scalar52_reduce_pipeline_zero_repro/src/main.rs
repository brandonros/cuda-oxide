//! PASSING-TWIN baseline for `dalek_from_canonical_bytes_zero_repro`
//! (DALEK-1 L0, slot 112). Full `Scalar::reduce()` pipeline composed
//! in THIS crate on all-zero input, using a byte-for-byte port of
//! `curve25519_dalek::backend::serial::u64::scalar` — `Scalar52`, `L`,
//! `R`, `from_bytes`, `mul_internal`, `montgomery_reduce`, `sub`,
//! `as_bytes`.
//!
//! Surfaced from vanity-miner-rs self_test slot 117. Per the data
//! this PASSes on device despite running byte-identical code to what
//! dalek's own `reduce()` body runs. That asymmetry — same algorithm
//! in-tree passes, cross-crate fails — is the smoking gun for the
//! cross-crate monomorphization bug behind every Bug-71 / DALEK-1
//! surface.
//!
//! ## What this DOES NOT depend on
//!
//! * `curve25519-dalek` — no `Scalar`, no `EdwardsPoint`, nothing.
//! * `subtle` — no `Choice`, no `CtOption`.
//! * `zeroize` — no Drop impls.
//!
//! The only crates touched at codegen time are `core` and the
//! `cuda-*` runtime. Any PTX shape this example does NOT contain
//! that the failing twin DOES contain is candidate-bug-surface.
//!
//! ## Build with
//!
...
/// Verbatim port of the subset of
/// `curve25519_dalek::backend::serial::u64::scalar` needed to run a
/// full `Scalar::reduce()` pipeline. Source: curve25519-dalek 4.1.3.
mod bisect_scalar52 {
...
/// Verbatim port of vanity-miner-rs `logic::check_dalek_reduce_pipeline_zero`.
///
/// Full `Scalar::reduce()` pipeline composed in *this* crate on
/// all-zero input. Identical body to real dalek's `reduce()`, but
/// every call resolves to the verbatim port above.
#[inline(never)]
pub fn check() -> u32 {
...
fn main() {
    println!("=== dalek_scalar52_reduce_pipeline_zero_repro (passing twin for slot 112) ===");
    ...
    if host_result[0] != 1 {
        ...
        eprintln!("If this fails, the bug has spread to the in-tree port path —");
        eprintln!("the diff strategy is no longer well-grounded; re-check assumptions.");
        std::process::exit(1);
    }

    println!();
    println!("SUCCESS: in-tree Scalar52 reduce pipeline matches on device (as expected).");
    println!("PASS");
}
